...
/*
Three things I would like to note about this: 1, I did not expect anybody to see it until I had a stable version
so my commenting/formatting is somewhat inconsistent in parts. 2, this board is currently set to play 2 AI players. 
3, It throws a seg fault whenever a piece controlled by the AI has to jump twice in one turn while the AI is thinking 
ahead. I believe the structure of the functions is causing it to not get any legal moves in the queue. Will be my 
focus next time I have a chance to work on it
*/
...
void checkers::comp_make_red_move()
{
	...
	for ( ; !(moves.empty()); moves.pop())
	{
		...
		tmp -> make_move(moves.front());
		tmp -> switch_player();
		tmp_value = tmp -> red_evaluate_possibilities(4, highest_value);
		delete tmp;
...
int checkers::red_evaluate_possibilities(int levels, int highest)
{
	...
	{
		queue<string> black_moves, red_moves;
		...
		for( ; !(black_moves.empty()); black_moves.pop())
		{
			...
			for( ; !(red_moves.empty()) ; red_moves.pop())
			{
				...
				tmp -> switch_player();
				value = tmp -> red_evaluate_possibilities(levels - 1, best_value);
				delete tmp;
				if (value > best_value)
				{
					if(-value <= highest)
...
void checkers::comp_make_black_move()
{
	...
	for ( ; !(moves.empty()); moves.pop())
	{
		...
		tmp -> make_move(moves.front());
		tmp -> switch_player();
		tmp_value = tmp -> black_evaluate_possibilities(4, lowest_value);
		delete tmp;
	...
	else
	{
		...
		for( ; !(red_moves.empty()); red_moves.pop())
		{
			...
			for( ; !(black_moves.empty()) ; black_moves.pop())
			{
				tmp -> make_move(black_moves.front());
				tmp -> switch_player();
				value = tmp -> black_evaluate_possibilities(levels - 1, best_value);
				delete tmp;
				if (value < best_value)
				{
					if(-value >= lowest)
